...
- **Role & Permission page** (`renderRoles`, header "🔑 สิทธิ์") is a read-only reference: capability matrix + the current user's role per project. Member/role assignment is done in-project via `openMembers(p)` (PI/admin only); admins are excluded from the "add member" list (they're superusers). The roles page also has a self-service "🔒 เปลี่ยนรหัสผ่าน" (`openMyPassword`).
